  ...
  const AdminView=()=>{
    ...
    const saveEdit=async(id)=>{
      ...
      try{
        ...
        const emailChanged=current && current.email!==eu.email.trim();
        const fields={name:eu.name.trim(),group:eu.group,role:eu.role};
        // L'email si passa solo se è cambiata: così cambia l'email di LOGIN
        // (via Edge Function) senza chiamate inutili quando resta uguale.
        if(emailChanged) fields.email=eu.email.trim();
        const updated=await updateProfile(id,fields);
        setUsers(p=>p.map(x=>x.id===id?{...x,...updated}:x));
        setEditId(null);
        showToast(emailChanged?"Utente aggiornato (email di login modificata)":"Utente aggiornato","success");
      }catch(err){
        console.error(err);
        showToast(err.message||"Errore nell'aggiornamento utente","error");
      }
    };
              ...
              <div key={u.id} style={{padding:"11px 18px",borderBottom:i<users.length-1?"1px solid #13161e":"none"}}>
                {editId===u.id ? (
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    ...
                    <input style={{...inp,flex:"1 1 160px"}} placeholder="Email (login)" value={eu.email}
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      {u.id!==me.id&&(
                        <button onClick={async()=>{
                            if(!window.confirm(`Eliminare definitivamente ${u.name}?\nVengono rimossi l'account di accesso e tutto lo storico presenze. L'operazione non è reversibile.`)) return;
                            try{ await deleteProfile(u.id); setUsers(p=>p.filter(x=>x.id!==u.id)); showToast("Utente eliminato","success"); }
                            catch(e){ console.error(e); showToast(e.message||"Errore eliminazione utente","error"); }
                          }}
                          style={{padding:"5px 9px",background:"transparent",border:"1px solid #2a2f45",
                            borderRadius:7,color:"#6b7280",fontSize:12,cursor:"pointer"}}>✕</button>
                      )}
